...
class Template{
  ...
  function backend($content, $data = NULL){
    ...
    function Pasien($content, $data = NULL){
        date_default_timezone_set('Asia/Jakarta');
        ini_set('date.timezone', 'Asia/Jakarta');
          // $level=$_SESSION['level'];
          // $level_menu=$_SESSION['level_menu'];
          
          $data['header'] = $this->_ci->load->view('template/pasien/header', $data, TRUE);
          $data['sidebar'] = $this->_ci->load->view('template/pasien/sidebar', $data, TRUE);
          $data['content'] = $this->_ci->load->view($content, $data, TRUE);
          $data['footer'] = $this->_ci->load->view('template/pasien/footer', $data, TRUE);
          
          $this->_ci->load->view('template/pasien/index', $data);
      }
}