...
class LapPatientAno extends MY_Controller
{
    ...
    {
       ini_set('memory_limit','356M'); // This also needs to be increased in some cases. Can be changed to a higher value as per need)
    ...
    public function export(){
      ...
      ini_set('pdo_sqlsrv.client_buffer_max_kb_size','524288'); // Setting to 512M - for pdo_sqlsrv
        ...
        $sheet->setCellValue('AD3', "Perintah Pulang To Discharge"); 
        $sheet->setCellValue('AE3', "Discharge To Jaminan"); 
        ...
        foreach($dataPatient as $data){ 
          ...
          $sheet->setCellValue('AD'.$numrow, floor($Jam).' Jam '.floor($Menit).' Menit'); 
          $Jam = $data->Discharge_To_Terima_Jaminan / 60;
          $Menit = $data->Discharge_To_Terima_Jaminan % 60;
          $sheet->setCellValue('AE'.$numrow, floor($Jam).' Jam '.floor($Menit).' Menit'); 
