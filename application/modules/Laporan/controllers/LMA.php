...
class LMA extends MY_Controller
{
    ...
    public function data_detail()
    {
        ...
        $this->dbDev->select();
        $this->dbDev->from("dbo.View_LMA_Regis");
            ...
            foreach ($dataFilter as $key => $value) {
                ...
                if ($value['field'] == 'date') {
                    if ($value['field'] == null || $value['field'] == '') {
                        ...
                        $where = " Date_Registrasi ".$value['type']." '".$value['value']."' ";
                        $this->dbDev->where($where);
        ...
        $this->dbDev->select();
        $this->dbDev->from("dbo.View_LMA_Regis");
        $this->dbDev->where(" Confirm_GP = '1' AND Confirm_CASEMIX = '0' AND ToFix = '0' ");
        ...
        // $this->dbDev->where("b.Status_Data <> 'In' ");

        if ($limit!= '') $this->dbDev->limit($limit, $start);

        if ($dataSort == NULL OR "") {
            $this->dbDev->order_by("Date_Registrasi", "DESC");
    ...
    public function CekTimeline()
    {
        ...
        $dataPasien = $this->dbDev->query("SELECT 
        A.Create_PDS, 
        ...
        $HistoryPasien = $this->dbDev->query("SELECT RTRIM(A.Ket_User) AS Ket_User, FORMAT(A.Date_User, 'yyyy-MM-dd') + ' ' + LEFT(A.Time_User, 2) + ':' + SUBSTRING(A.Time_User, 3, 2) + ':' + RIGHT(A.Time_User, 2) AS DateUser, 
        A.Action, 
        (
            IIF(
    ...
    public function GetDetailDataPasien()
    {
        ...
        $DataPasienLMA = $this->dbDev->query("SELECT * FROM dbo.View_LMA_Regis A WHERE A.No_Regis = '$object->Ip_Registration_No'")->row();
        // $DataPasienLML = $this->dbDev->query("SELECT B.Patient_Name, A.* FROM DB_Transaction_Fix.dbo.Header_Doc_LML A LEFT JOIN DB_Master_Fix.dbo.Pasien B ON B.MR_No = A.MR_No
    ...
    public function ApproveLMA()
    {
        ...
        $q= $this->dbDev->query("SELECT * FROM dbo.Laporan_Medis_Awal WHERE No_Regis = '$No_Regis'")->row();
        ...
        if ($Position_Status == 3) {
            ...
            $q2= $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.With_Notes_LMA WHERE No_MR = '$q->No_MR' AND No_Regis = '$No_Regis'")->row();
            $DataWithNote = array(
                'Branch' => $q->Branch,
                'No' => $q2->No + 1,
                'No_MR' => $q->No_MR,
            ...
            $this->dbDev->insert('DB_Transaction_Fix.dbo.With_Notes_LMA', $DataWithNote);
        ...
        $this->dbDev->insert('dbo.History_Trx_Dok_LMA',$DataLog);
    ...
    public function ApproveLMAtoFix()
    {
        ...
        // $this->dbDev->where('No_Regis', $No_Regis);
        // $this->dbDev->update('dbo.Laporan_Medis_Awal', $DataUpdate);
