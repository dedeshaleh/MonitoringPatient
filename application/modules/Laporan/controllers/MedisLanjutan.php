...
class MedisLanjutan extends MY_Controller
{
    //
    public $CI;
    ...
    {
        // Example
        $data['judul'] = "Laporan Medis Lanjutan";

        if ($this->session->userdata('id_level') == '5') {
            $data['Ward'] = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.RI_Ward WHERE Active = '1' AND Description IN ('ICU')")->result();
        } else if ($this->session->userdata('id_level') == '6') {
            $data['Ward'] = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.RI_Ward WHERE Active = '1' AND Ward_Code IN ('CAE', 'CAE2')")->result();
        } else if ($this->session->userdata('id_level') == '7') {
            $data['Ward'] = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.RI_Ward WHERE Active = '1' AND Ward_Code IN ('MGD', 'EMM', 'DRB')")->result();
        } else if ($this->session->userdata('id_level') == '8') {
            $data['Ward'] = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.RI_Ward WHERE Active = '1' AND Ward_Code IN ('NZR', 'JOP')")->result();
        }else{
            $data['Ward'] = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.RI_Ward WHERE Active = '1' AND Description NOT IN ('BEREA', 'HCU', 'ISOLATION', 'NICU', 'OPERATING THEATER', 'PERINATOLOGI', 'PHANTOM WARD', 'ICCU', 'PICU', 'MORIAH ICU')")->result();
        }


        $this->template->backend('./LaporanMedis/LML/v_index', $data);
    }
    public function data_detail()
    {
        ...
        $this->dbDev->select();
        $this->dbDev->from("dbo.Vw_MedisLanjutan");
            ...
            foreach ($dataFilter as $key => $value) {
                ...
                if ($value['field'] == 'date') {
                        if ($value['value'] == null || $value['value'] == '') {
                            ...
                            $where = " Date ".$value['type']." '".$value['value']."' ";
                            $this->dbDev->where($where);
        ...
        // var_dump($where);
        // exit();
        // $this->dbDev->where("Status_Data != 'In' ");

        

        $count = $this->dbDev->count_all_results();

        // $cek = $this->dbDev->last_query();
        // echo $count;
        // echo $cek;
        ...
        // $this->dbDev->where("a.Position_Status in ('2') ");

        if ($limit!= '') $this->dbDev->limit($limit, $start);

        if ($dataSort == NULL OR "") {
            $this->dbDev->order_by("Reg_No", "DESC");
            // $this->dbDev->order_by("A.File_No", "DESC");

        }else{
            foreach ($dataSort as $key => $value) {
                $this->dbDev->order_by($value['field'], $value['dir']);
            }
        }
    ...
    public function DataChart()
    {
        ...
        $q1 = $this->dbDev->query("SELECT COUNT(*) AS Total FROM DB_Transaction_Fix.dbo.View_TrackingPatient WHERE Perintah_Pulang_To_Discharge > 120 AND ( MONTH(Date_Registration) = $Bulan and YEAR(Date_Registration) = $Tahun )")->row();
        $q2 = $this->dbDev->query("SELECT COUNT(*) AS Total FROM DB_Transaction_Fix.dbo.View_TrackingPatient WHERE Discharge_To_Terima_Jaminan > 60 AND ( MONTH(Date_Registration) = $Bulan and YEAR(Date_Registration) = $Tahun)")->row();
        $q3 = $this->dbDev->query("SELECT COUNT(*) AS Total FROM DB_Transaction_Fix.dbo.View_TrackingPatient WHERE (Perintah_Pulang_To_Discharge <= 120 AND Discharge_To_Terima_Jaminan <= 60) AND ( MONTH(Date_Registration) = $Bulan and YEAR(Date_Registration) = $Tahun )")->row();
        $q4 = $this->dbDev->query("SELECT COUNT(*) AS Total FROM DB_Transaction_Fix.dbo.View_TrackingPatient WHERE MONTH(Date_Registration) = $Bulan and YEAR(Date_Registration) = $Tahun ")->row();
        $q5 = $this->dbDev->query("SELECT COUNT(*) AS Total FROM DB_Transaction_Fix.dbo.View_TrackingPatient WHERE (Perintah_Pulang_To_Discharge > 120 OR Discharge_To_Terima_Jaminan > 60) AND MONTH(Date_Registration) = $Bulan and YEAR(Date_Registration) = $Tahun ")->row();
        $data = array(
            'SLA1' => $q1->Total,
            'SLA2' => $q2->Total,
            'SLA3' => $q3->Total,
    ...
    public function DataDetailPasien()
    {
        ...
        $q = $this->dbDev->query("SELECT * FROM View_Journey_RI AS a WHERE IP_Registration_No = '$IP_Registration_No'")->row();
        $q2 = $this->dbDev->query("SELECT  DISTINCT(ax.Doctor), ax.IP_Registration_No, ax.DoctorType, bx.Doctor_Name FROM (
    ...
    public function ExportExcel()
    {
        ...
        $this->dbDev->select("Patient_Name, WARD, Date_Perintah_Pulang, Date_Discharge, Date_Terima_Jaminan, MR, RTRIM(LTRIM(Company_Name)) AS Company_Name");
        $this->dbDev->from("DB_Transaction_Fix.dbo.View_Journey_RI");
        $this->dbDev->where("Status_Data != 'In' ");
        if ($date_awal != null && $date_akhir != null) {
            $this->dbDev->where("(Date_Registration BETWEEN '$date_awal' AND '$date_akhir')");
        }else if ($date_awal != null) {
            $this->dbDev->where("Date_Registration = '$date_awal'");
        }else if ($date_akhir != null) {
            $this->dbDev->where("Date_Registration = '$date_akhir'");
        }
        if ($ward == '' || $ward == null) {
            
        }else{
            $this->dbDev->where('WARD', $ward);
        }
        $this->dbDev->order_by("Date_Terima_Jaminan", "ASC");
        $this->dbDev->order_by("Date_Discharge", "ASC");
        $this->dbDev->order_by("Date_Perintah_Pulang", "DESC");
        $q = $this->dbDev->get()->result();
    ...
    {
        $q = $this->dbDev->query("SELECT * FROM Backoffice.dbo.tbl_akses_menu")->result();
        echo json_encode($q);
    }
    ...
    public function CekTimeline()
    {
        ...
        $HistoryPasien = $this->dbDev->query("SELECT A.Action, A.Create_User, 
    ...
    public function GetDetailDataPasien()
    {
        ...
        // $DataPasienLMA = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.Laporan_Medis_Awal A WHERE A.No_Regis = '$object->Ip_Registration_No'")->row();
        $DataPasienLML = $this->dbDev->query("SELECT C.Doctor_Name, B.Company_Name, A.* FROM DB_Transaction_Fix.dbo.RI_Tran_Registrasi A
        ...
        WHERE A.IP_Registration_No = '$object->Ip_Registration_No'")->row();
         $DataDokumen = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.Detail_Doc_LML 
         WHERE Reg_No = '$object->Ip_Registration_No' AND File_No = '$object->File_No' ORDER BY Seq_No DESC")->row();
        // $DataPasien = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.Pasien A WHERE A.No_Regis = '$object->Ip_Registration_No'")->row();
    ...
    public function ConvertData($IP_Registration_No, $KodeDoc)
    {
        ...
        require('assets/fpdf/fpdf.php');
        $DataImg = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.Temp_Upload_Doc_LML WHERE Reg_No = '$IP_Registration_No' AND File_No = '$KodeDoc' ")->result();
        $DataDetail = $this->dbDev->query("SELECT TOP 1 * FROM DB_Transaction_Fix.dbo.Detail_Doc_LML WHERE Reg_No = '$IP_Registration_No' AND File_No = '$KodeDoc' ORDER BY Seq_No DESC ")->row();
    ...
    public function ApproveLML()
    {
        ...
        $q = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.Header_Doc_LML WHERE Reg_No = '$Reg_No' AND File_No = '$File_No'")->row();
       

        $qdata = $this->dbDev->query("SELECT * FROM DB_Master_Fix.dbo.Multi_Parameter_Detail WHERE Master_Code = 'STLML' AND Code = '$Position_Status'")->row();
        $CekHistory = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.History_Download_view_Doc WHERE No_Registration = '$Reg_No' ORDER BY Seq_No DESC")->row();
        // var_dump($CekHistory);
        $DataHistory = array(
            'Branch' => $q->Branch,
            'File_No' => $q->File_No,
    ...
    public function GetListDokumen()
    {
        ...
        $Data = $this->dbDev->query("SELECT * FROM DB_Transaction_Fix.dbo.Detail_Doc_LML WHERE Reg_No = '$object->IP_Registration_No' AND File_No = '$object->File_No'")->result();
        echo json_encode($Data);
    }
