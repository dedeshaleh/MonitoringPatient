...
class M_dashboard extends CI_Model {
    ...
    private function _get_datatables_query($term = '',$date_awal=null,$date_akhir=null,$data_search=null,$data_payer=null, $ward=null)
    {
       
        $column = array("", "a.Patient_Name", "a.Ward", "a.Date_Perintah_Pulang", "a.Date_Discharge", "a.Date_Terima_Jaminan", "a.MR", "a.Company_Name"); //Sesuaikan dengan field
        $this->dbProd23->select("a.IP_Registration_No, a.Patient_Name, a.Ward, a.Date_Perintah_Pulang, a.Date_Discharge, a.Date_Terima_Jaminan, a.MR, a.Company_Name");
        $this->dbProd23->from("View_Journey_RI AS a");
        
        if ($this->session->userdata('id_level') == '5') {
            $this->dbProd23->where(" a.Ward = 'IC' ");
        }
        if ($this->session->userdata('id_level') == '6') {
            $this->dbProd23->where(" (a.Ward = 'CAE' OR a.Ward = 'CAE2') ");
        }
        if ($this->session->userdata('id_level') == '7') {
            $this->dbProd23->where(" (a.Ward = 'MGD' OR a.Ward = 'EMM' OR a.Ward = 'DRB') ");
        }
        if ($this->session->userdata('id_level') == '8') {
            $this->dbProd23->where(" ( a.Ward = 'NZR' OR a.Ward = 'JOP' OR a.Ward = 'JOP2' ) ");
        }
        ...
        // }
        if ($date_awal != null && $date_akhir != null) {
            $this->dbProd23->where("( convert(varchar, Date_Registration, 23) BETWEEN '$date_awal 00:00:00' AND '$date_akhir 23:59:00')");
        }else if ($date_awal != null) {
            $this->dbProd23->where(" convert(varchar, Date_Registration, 23) = '$date_awal'");
        }else if ($date_akhir != null) {
            $this->dbProd23->where(" convert(varchar, Date_Registration, 23) = '$date_akhir'");
        ...
        $this->dbProd23->where(" a.Ward = '$ward' ");
        }
    ...
    function get_datatables($date_awal=null,$date_akhir=null,$data_search=null,$data_payer=null,$ward=null)
    {
        $term = $_REQUEST['search']['value'];
        $this->_get_datatables_query($term,$date_awal,$date_akhir,$data_search,$data_payer,$ward);
        if ($_POST['length'] != -1)
    ...
    function count_filtered($date_awal=null,$date_akhir=null,$data_search=null,$data_payer=null, $ward=null)
    {
        $term = $_REQUEST['search']['value'];
        $this->_get_datatables_query($term,$date_awal,$date_akhir,$data_search,$data_payer, $ward);
        $query = $this->dbProd23->get();
