          ...
          <div class="row">
                <div class="col-sm-2">
                  <div class="form-group">
                    ...
                    <input type="date" name="date_awal" id="date_awal" class="form-control" value="<?=date("Y-m-01", strtotime($today . " -1 month"))?>">
                  </div>
                </div>
                <div class="col-sm-2">
                  <div class="form-group">
      ...
      <table>
        <tr>
          ...
          <td> <span id="NamaPatient_LMA"></span> <input type="hidden" name="No_Regis" id="Reg_No_LMA"></td>
            ...
            <div style="overflow: auto;" id="canvasContainer"></div>
            <br>
          <div class="form-group">
          ...
          <span id="VerifPdf" style="color: red; text-align:right;" hidden>Maaf tidak bisa di Submit, karena dokumen '<span id="NamaFile_LMA"></span>' tidak ditemukan <br> (Silahkan hubungi IT)</span>
      </div>
     
      <div class="modal-footer">
        <button type="submit" class="btn btn-success btn-sm" id="SubmitApproveData">Submit</button>
        <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
...
<script>
                      ...
                      var TblPatientData = new Tabulator("#TblPatient", {
                        ...
                        initialFilter:[
                              { field:"No_Regis", type:"like", value:""},
                              { field: 'date', type: '>=', value: "<?=date("Y-m-01", strtotime($today . " -1 month"))?> 00:00:00" },
                              { field: 'date', type: '<=', value: "<?=date("Y-m-d")?> 23:59:59" }
                          ],
                        initialSort:[
                              {column:"No_Regis", dir:"asc"}, //sort by this first
                          ],
												columns:[
                          ...
                          {title:"Tanggal Registrasi" , sortable:false, field:"Tanggal_Registrasi", width:150,
														formatter:function(cell, formatterParams, onRendered){
                              var value = cell.getRow().getData();
															return FormatDate(value.Tanggal_Registrasi);
														},
													},
													{title:"IP Registrasi", field:"No_Regis", headerFilter:"input", cssClass: "custom-padding" },
													{title:"ER Registrasi", field:"No_Reg_ER", headerFilter:"input", cssClass: "custom-padding" },
...
function FormApproveLMA(Ip_Registration_No, File_No) {
    ...
    .then((data) => {
      if (data.DataPDF == null || data.DataPDF == "") {
        document.getElementById('canvasContainer').innerHTML = ''
        document.getElementById("SubmitApproveData").disabled = true
        document.getElementById("VerifPdf").hidden = false
      }else{
        document.getElementById("SubmitApproveData").disabled = false
        document.getElementById("VerifPdf").hidden = true
      ...
      pdfjsLib.GlobalWorkerOptions.workerSrc = '<?=base_url()?>assets/pdfjs/build/pdf.worker.js';
      ...
      pdfjsLib.getDocument({data: pdfData}).promise.then(function(pdfDoc_) {
        ...
        // }
        pdfDoc = pdfDoc_;
        // Jumlah canvas yang ingin Anda buat
        const numberOfCanvases = pdfDoc.numPages;
        // Loop untuk membuat dan menambahkan canvas ke dalam kontainer
        for (let i = 1; i <= numberOfCanvases; i++) {
          // Buat elemen canvas baru
          const canvas = document.createElement('canvas');

          // Dapatkan konteks canvas untuk menggambar di dalamnya
          ...
          // Menambahkan elemen canvas ke dalam kontainer
    ...
    document.getElementById('NamaFile_LMA').innerHTML = data.DataLMA.PDF_FIle
    // console.log(data.DataLMA)
